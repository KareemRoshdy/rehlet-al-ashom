...
interface ImageFormProps {
  initialData: DaleyRecommendation;
  recommendationId: string;
}

const formSchema = z.object({
  imageUrl: z.string().min(1, { message: "صورة الباقة مطلوبة" }),
});

const ImageForm = ({ initialData, recommendationId }: ImageFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.patch(
        `/api/daily-recommendations/${recommendationId}`,
        values
      );
